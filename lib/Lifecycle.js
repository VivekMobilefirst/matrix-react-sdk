...
var _VideoChannelStore = _interopRequireDefault(require("./stores/VideoChannelStore"));
...
async function startMatrixClient() {
  ...
  _CallHandler.default.instance.start(); // Start Mjolnir even though we haven't checked the feature flag yet. Starting
  ...
  _Jitsi.Jitsi.getInstance().start(); // In case we disconnected uncleanly from a video room, clean up the stuck device


  if (_VideoChannelStore.default.instance.roomId) {
    (0, _VideoChannelUtils.fixStuckDevices)(_MatrixClientPeg.MatrixClientPeg.get().getRoom(_VideoChannelStore.default.instance.roomId), false);
...
function stopMatrixClient() {
  ...
  _CallHandler.default.instance.stop();
